...
export const movePlayer = ({delta, position, shape, board}) => {
    ...
    const collided = hasCollision({
        board,
        position: futureNextPosition,
        shape});
    const onTheBoard = isWithinBoard({
        board,
        position: futureNextPosition,
        shape});

    const preventMovement = !onTheBoard || (onTheBoard && collided);
    const nextPosition = preventMovement ? position : futureNextPosition;

    const isMovingDown = delta.row > 0;
    const isHit = isMovingDown && (collided || !onTheBoard);
